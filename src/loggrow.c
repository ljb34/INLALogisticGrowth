...
void a_func(double growth, double carry_cap,
	double* linpoint, int ns, int nt, double* result) { /*important! must give pointer for where to export result and supply length of linpoint (n)*/
...
void r_vector(double growth, double carry_cap, double move_const,
    double* linpoint, double* grad, int ns, int nt, double* result) {
    for (int i = 0; i < ns; i++) {
        for (int t = 0; t < nt; t++) {
            int idx = t * ns + i;
            mag_grad_sq = grad[2 * idx] * grad[2 * idx] + grad[2 * idx + 1] * grad[2 * idx + 1];
            lp = linpoint[idx];
            result[idx] = growth * exp(lp) * (lp - 1) / carry_cap + growth - move_const * mag_grad_sq;
        }
    }
}
...
double* inla_cgeneric_loggrow_model(inla_cgeneric_cmd_tp cmd, double* theta, inla_cgeneric_data_tp* data) {
	...
	double* ret = NULL, prec = (theta ? exp(theta[0]) : NAN), lprec = (theta ? theta[0] : NAN); //interpret.theta equivalent
	assert(!strcasecmp(data->ints[0]->name, "n")); // this will always be the case
	int N = data->ints[0]->ints[0]; // this will always be the case
	assert(N > 0);

	// Read parameters from data - figure out how this gets passed in


	switch (cmd) { //different outputs for the comands supplied
    ...
    case INLA_CGENERIC_MU:
    {
		...
		rvector->len = N;
		r_vector(growth, carry_cap, move_const, linpoint, grad, ns, nt, rvector->doubles);
