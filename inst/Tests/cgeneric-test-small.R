...
library(sf)
out.lgcp <- simulate_loggrowth(growth = 0.8, k = 50, movement = 1, sigma = 2,
                               initial = 25,timesteps = 4,sample.type = "LGCP")
...
priors <- list(cc = c(log(nrow(out.lgcp$animal_obs[out.lgcp$animal_obs$time == 4,])),1),
               growth = c(0.8,1),move = c(1,1),sigma = c(log(2),1))
...
saveRDS(iterated.fit.lgcp, "LogGrowth/cgeneric_small.RData")
...
preds <- predict(iterated.fit.lgcp$fit, pred.pixels.time,
                 ~data.frame(time = 1:4, loglambda = loggrow,
                             lambda = exp(loggrow)),
                 n.samples = 100)
saveRDS(preds, "LogGrowth/cgeneric_small_preds.RData")
