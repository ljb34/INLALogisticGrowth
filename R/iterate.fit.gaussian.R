...
iterate.fit.gaussian <- function(data, smesh, tmesh, samplers,prior.mean,
  ...
  if(max(new.linpoint)-min(new.linpoint) >= 10000*(max(initial.linpoint)-min(initial.linpoint))){#if new lin point is extremely different
    ...
    initial.fit <- fit
    initial.linpoint2 <- log(logit.nest(exp(prior.mean) + rnorm(smesh$n*tmesh$n, sd = 0.1), initial.growth, 1/exp(initial.inv.carry.cap), tmesh$n - 1)$x)
    ...
    if(!is.na(n.nodes)){
      ...
      new.mean <- Reduce("+", weighted.means)
      new.linpoint <- (1-gamma)*initial.linpoint +gamma*new.mean
      if(max(new.linpoint)-min(new.linpoint) >= 10000*(max(initial.linpoint)-min(initial.linpoint))){
        print("New linearisation point is still strange, returning model output")
        return(list(fit = fit, initial.linpoint = initial.linpoint, new.linpoint = new.linpoint, initial.fit = initial.fit))
      }
    }
  ...
  while(n < max.iter & mean(abs(lp.mat[,n]-lp.mat[,n-1]))>stop.crit){
    log_growth_model <- define.loggrow.model(linpoint = as.vector(new.linpoint), 
                                             ...
                                             #initial.inv.carry.cap = fit$summary.hyperpar$mean[3],
                                             #initial.move.const = fit$summary.hyperpar$mean[4],
                                             #initial.log.sigma = fit$summary.hyperpar$mean[5])
                                             initial.growth = initial.growth, 
                                             initial.inv.carry.cap = initial.inv.carry.cap,
                                             initial.move.const = initial.move.const,
                                             initial.log.sigma = initial.log.sigma)
