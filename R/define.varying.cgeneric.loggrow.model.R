#'@name define.varying.cgeneric.loggrow.model
...
#'@param growth.formula formula for growth
...
#'@export
define.varying.cgeneric.loggrow.model <- function(linpoint, smesh, tmesh, step.size,
                                          prior.mean, prior.precision, growth.formula,
                                          carry.formula, move.formula, growth_cov,
                                          carry_cov, move_cov,
  ...
  the_model <- do.call("inla.cgeneric.define",args0)
                       #c(args0,
                         # list(ns = as.integer(smesh$n),
                         #      nt = as.integer(tmesh$n),
                         #      Pn = as.integer(length(PG$graph@x)),
